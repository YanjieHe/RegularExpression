...
namespace regex {
    ...
    enum class RangeType { Epsilon, LineBegin, LineEnd, CharacterRange };
    ...
    struct UnicodeRange {
        ...
        UnicodeRange() : rangeType{RangeType::Epsilon}, lower{0}, upper{0} {}
        UnicodeRange(RangeType rangeType, char32_t lower, char32_t upper)
            : rangeType{rangeType}, lower{lower}, upper{upper} {}
    ...
    inline bool operator==(const UnicodeRange& x, const UnicodeRange& y) {
        return x.rangeType == y.rangeType && x.lower == y.lower && x.upper == y.upper;
    }
    ...
    public:
        ...
        Edge() = default;
        Edge(StateID from, StateID to, UnicodeRange pattern) : from{from}, to{to}, pattern{pattern} {}
        bool IsEpsilon() const {
    ...
    public:
        ...
        DFATableRow(std::set<StateID> index, vector<std::set<StateID>> nextStates)
            : index{index}, nextStates{nextStates} {}
    };
