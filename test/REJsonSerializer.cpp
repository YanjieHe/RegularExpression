...
Json REJsonSerializer::VisitAlternation(const AlternationExpression::Ptr& exp) {
    return {{"kind", "Alternation"}, {"left", VisitRegularExpression(exp->left)},
        {"right", VisitRegularExpression(exp->right)}};
}

Json REJsonSerializer::VisitConcatenation(const ConcatenationExpression::Ptr& exp) {
    return {{"kind", "Concatenation"}, {"left", VisitRegularExpression(exp->left)},
        {"right", VisitRegularExpression(exp->right)}};
}
...
Json REJsonSerializer::VisitSymbol(const SymbolExpression::Ptr& exp) {
    ...
    return {{"kind", "Symbol"}, {"lower", utf8::utf32to8(u32string({exp->range.lower}))},
